...
function partialMatch(quality: string): string[] {
  // Return all G chord keys that contain the quality substring (loose match)
  const keys = Object.keys(G_CHORDS);
  // For example, user typed Gma -> root=G, quality='ma'
  // Find all keys starting with G and whose name (after G) contains 'ma'
  // We'll do a simple substring search on the full chord name (including G).
  return keys.filter(
    (k) => k.startsWith("G") && k.toLowerCase().includes(quality.toLowerCase())
  );
}

function transposeChordShape(
  gShape: ChordShape,
  fromRoot: string,
  toRoot: string
): ChordShape | null {
...
export function getChordShapes(
  chordName: string
): { chordName: string; shapes: ChordShape[] }[] {
  const parsed = parseChordName(chordName);
  if (!parsed) return [];
  const { root, quality } = parsed;

  // Get all partial matches for the quality if exact match is not found
  const matchedChords = partialMatch(quality);
  if (matchedChords.length === 0) return [];

  const result: { chordName: string; shapes: ChordShape[] }[] = [];

  // If root is G, just use the shapes from G_CHORDS
  // Otherwise transpose each chord
  matchedChords.forEach((gQuality) => {
    const originalName = gQuality; // e.g. Gmaj7
    // If root != G, transpose chord name (just replace G with root for display)
    const displayName =
      root === "G" ? originalName : root + originalName.substring(1);

    const shapes = G_CHORDS[gQuality]
      .map((shape) => {
        if (root === "G") return shape;
    ...
    if (shapes.length > 0) {
      result.push({ chordName: displayName, shapes });
    }
  });
